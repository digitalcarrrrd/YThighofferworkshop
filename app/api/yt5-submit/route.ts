    ...
    if (screenshotBase64 && typeof screenshotBase64 === "string" && screenshotBase64.length > 50 && token) {
      try {
        const uploaded = await uploadToGhlMedia(
          screenshotBase64,
          screenshotFilename || `yt5_receipt_${Date.now()}.jpg`,
          token
        );
        if (uploaded) {
          receiptUrl = uploaded;
          console.log("[YT5 SUBMIT] Stored receipt to GHL Media CDN:", receiptUrl);
        }
      } catch (e) {
        console.warn("[YT5 SUBMIT] Receipt storage error:", e);
      }
    }
    ...
    if (token && locationId) {
      try {
        ...
        const contactPayload: any = {
          ...
          source: "Landing Page: /workshops/yt5",
          tags,
          customFields: [
            ...
            ...(process.env.GHL_LANDING_PAGE_FIELD_KEY
              ? [{ key: process.env.GHL_LANDING_PAGE_FIELD_KEY, field_value: "/workshops/yt5" }]
              : []),
        ...
        if (contactId) {
          const noteLines = [
            `📸 [PAYMENT SCREENSHOT ATTACHED VIA WEBSITE FORM - WORKSHOP 5]`,
